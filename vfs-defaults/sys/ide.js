...
const IDE = {
    ...
    init() {
        ...
        fluxide.keybinds.register('ide.undo', 'Ctrl-Z', () => { if(this.cm && !this.cm.getOption('readOnly')) this.cm.undo(); }, "Undo Edit");
        fluxide.keybinds.register('ide.redo', 'Ctrl-Y', () => { if(this.cm && !this.cm.getOption('readOnly')) this.cm.redo(); }, "Redo Edit");
    ...
    openFile(path) {
        ...
        if (!this.docCache.has(path)) {
            const content = state.get().vfs[path] || "";
            const ext = path.split('.').pop();
            const doc = CodeMirror.Doc(content, this.getExtMode(ext));
        ...
        const baseHeaderTools = [
            h('button', { class: 'fx-icon-btn', title: 'Refresh File System', innerHTML: theme.getIcon('refresh'), onClick: async () => {
                ...
                if(this.activeTab && files[this.activeTab] !== undefined) {
                    if (this.docCache.has(this.activeTab)) {
                        this.docCache.get(this.activeTab).setValue(files[this.activeTab]);
                    } else if (this.cm) {
    ...
    renderTabs() {
        ...
        const openNames = this.openTabs.map(p => p.split('/').pop());
        const hasDupes = (name) => openNames.filter(n => n === name).length > 1;
                        ...
                        if(isFile) {
                            menu.push({ label: 'Rename File', icon: theme.getIcon('pencil'), action: async () => {
                                ...
                                if(newBaseName && newBaseName !== currentName) {
                                    ...
                                    if (this.docCache.has(itemPath)) {
                                        this.docCache.set(newPath, this.docCache.get(itemPath));
                                        this.docCache.delete(itemPath);
                                    }
                                ...
                                if(newBaseName && newBaseName !== currentName) {
                                    ...
                                    Object.keys(state.get().vfs).forEach(async p => {
                                        if(p.startsWith(itemPath + '/')) {
                                            ...
                                            if (this.docCache.has(p)) {
                                                this.docCache.set(targetPath, this.docCache.get(p));
                                                this.docCache.delete(p);
                                            }
                                    ...
                                    if (isFile) {
                                        ...
                                        Object.keys(state.get().vfs).forEach(async p => {
                                            if(p.startsWith(itemPath + '/')) {
                                                ...
                                                if (this.docCache.has(p)) {
                                                    this.docCache.set(newP, this.docCache.get(p));
                                                    this.docCache.delete(p);
                                                }
